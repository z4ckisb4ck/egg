...
def compute_percentile_intervals(preds, levels=(90, 95, 99)):
    """
    preds: array, shape (n_bootstrap, n_periods)
    returns: mean (n_periods,), dict[level] = (lower, upper)
    """
    mean = preds.mean(axis=0)
    intervals = {}
    for lvl in sorted(levels, reverse=True):
        lo = np.percentile(preds, (100 - lvl) / 2, axis=0)
        hi = np.percentile(preds, 100 - (100 - lvl) / 2, axis=0)
        intervals[lvl] = (lo, hi)
    return mean, intervals

def compute_calibration(y_true, preds, level=99):
    """
    y_true: array-like, shape (n_periods,)
    preds: array, shape (n_bootstrap, n_periods)
    level: int, e.g. 99
    returns fraction of y_true[i] inside the level% CI
    """
    lo, hi = compute_percentile_intervals(preds, [level])[1][level]
    inside = ((y_true >= lo) & (y_true <= hi)).mean()
    return inside
config = {
    'learning_rate': st.sidebar.number_input(
        'Learning rate', min_value=0.001, max_value=1.0,
        value=st.session_state.get('learning_rate', 0.1), step=0.001
    ),
    'n_estimators': st.sidebar.number_input(
        'Number of estimators', min_value=100, max_value=2000,
        value=st.session_state.get('n_estimators', 1000), step=50
    ),
    'lambda_reg': st.sidebar.number_input(
        'Lambda regularization', min_value=0.0, max_value=10.0,
        value=st.session_state.get('lambda_reg', 1.0), step=0.1
    ),
    'alpha_reg': st.sidebar.number_input(
        'Alpha regularization', min_value=0.0, max_value=10.0,
        value=st.session_state.get('alpha_reg', 0.0), step=0.1
    )
}
adjusted_lr = config['learning_rate']

def backtest_and_plot(df, features, target_col,
                     bootstrap_fn,
                     start_idx=None, end_idx=None,
                     ci_levels=(90, 95, 99),
                     title="Egg Price Forecast"):
    """
    df: full DataFrame with date sorted ascending and target_col
    features: list of feature column names
    bootstrap_fn: function(train_df, test_df) -> preds array (n_bootstrap, n_periods)
    start_idx/end_idx: slice of df indices to back-test on
    """
    dates = df.index if isinstance(df.index, pd.DatetimeIndex) else pd.to_datetime(df.date)
    if start_idx is None: start_idx = int(len(df) * 0.5)
    ...
    for i in range(start_idx, end_idx):
        train = df.iloc[:i]
        test  = df.iloc[i:i+1]
        preds = bootstrap_fn(train, test, features)
        # preds shape (n_bootstrap, 1)
        y_trues.append(test[target_col].values[0])
        all_preds.append(preds.flatten())

    # stack into array of shape (n_bootstrap, n_periods)
    all_preds = np.stack(all_preds, axis=1)
    y_trues   = np.array(y_trues)

    # compute mean & intervals
    mean, intervals = compute_percentile_intervals(all_preds, ci_levels)
    cal99 = compute_calibration(y_trues, all_preds, level=99)

    # plot
    plt.figure(figsize=(10, 4))
    for lvl in sorted(ci_levels, reverse=True):
        lo, hi = intervals[lvl]
        plt.fill_between(dates[start_idx:end_idx], lo, hi, alpha=0.2, label=f"{lvl}% CI")
    plt.plot(dates[start_idx:end_idx], mean, label="Predicted mean")
    plt.plot(dates[start_idx:end_idx], y_trues,  label="Actual", linestyle='--', marker='o', markersize=3)
    plt.title(title)
    plt.xlabel("Date")
    ...
    st.write(f"**Calibration:** {cal99*100:.1f}% of actuals fell inside the 99% confidence band")

# ————————————————————————————————
# Then, in your main Streamlit flow, replace your old CI plot with:

if st.sidebar.button("Run back-test & CI"):
    backtest_and_plot(
        df=st.session_state['df'],
        features=st.session_state['features'],
        target_col="price",                # or whatever your target column is
        bootstrap_fn=lambda tr, te, feats: bootstrap_predict(tr, te, feats,
                                                             n_bootstrap=50,
                                                             learning_rate=adjusted_lr,
                                                             n_estimators=config['n_estimators'],
                                                             lambda_reg=config['lambda_reg'],
                                                             alpha_reg=config['alpha_reg']),
        start_idx=len(st.session_state['df'])-12,   # last 12 months
        ci_levels=(90, 95, 99),
        title="Egg Price back-test and 90/95/99% CIs"
    )
